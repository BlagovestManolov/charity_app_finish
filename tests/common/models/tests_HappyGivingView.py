...
from charity_app.common.views import HappyGivingView


class VolunteerTest(CreateObjectsMixin, TestCase):
    volunteer = HappyGivingView()

    def setUp(self) -> None:
        self.url = reverse('happy-giving')
    ...
    def test__template_used__expect_correct(self):
        response = self.client.get(self.url)
        self.assertTemplateUsed(response, 'common/happy-giving-page.html')
